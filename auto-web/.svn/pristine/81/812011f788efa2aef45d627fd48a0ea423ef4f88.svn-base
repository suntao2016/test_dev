 ...
 * 搜索 已卖出物品--等待发货
 */
public class WaitingShipGoodsStep extends SupplierStep {
	/**
	 * slf4j
	 */
	...
	/**
	 * test test pc supplier 已卖出的物品--等待发货
	 * @throws Exception 
	 */
	@Test(description = "test pc supplier 已卖出的物品--等待发货")
	public void testWaitingShip()  {    		    	
		    try{
		    	logger.info("test pc supplier 已卖出的物品--等待发货测试用例执行开始，测试用例名称是：{}",getClass().getName());
		    	/**
		    	 * 初始化所需对象
		    	 */
		    	...
		    	Assert.assertEquals("待发货",statu.trim());
		    	logger.info("test pc supplier 已卖出的物品--等待发货订单测试用例执行结束！");
		    }catch (AssertionError e) {
				logger.error("Constant.TEST_SUPPLIER_WAITING_SHIP_FAILL ", e);
				Assert.fail(Constant.TEST_SUPPLIER_WAITING_SHIP_FAIL + " >> "
						+ e.getMessage());
			} catch (Exception e) {
				logger.error(Constant.TEST_SUPPLIER_WAITING_SHIP_FAIL, e);
				throw new TestNGException("Constant.TEST_SUPPLIER_WAITING_SHIP_FAIL >> "
						+ e.getMessage(), e);
			}
