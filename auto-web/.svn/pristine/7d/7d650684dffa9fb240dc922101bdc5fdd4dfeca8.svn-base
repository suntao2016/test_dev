 ...
 * 搜索 已卖出物品--已完成
 */
public class CompletedGoodsStep extends SupplierStep {
	/**
	 * slf4j
	 */
	...
	/**
	 * test test pc supplier 已卖出的物品--已完成
	 * @throws Exception 
	 */
	@Test(description = "test pc supplier 已卖出的物品--已完成")
	public void testShipedGoods()  {    		    	
		    try{
		    	logger.info("test pc supplier 已卖出的物品--已完成测试用例执行开始，测试用例名称是：{}",getClass().getName());
		    	/**
		    	 * 初始化所需对象
		    	 */
		    	...
		    	Assert.assertEquals("已完成",statu.trim());
		    	logger.info("test pc supplier 已卖出的物品--已完成订单测试用例执行结束！");
		    }catch (AssertionError e) {
				logger.error("Constant.TEST_SUPPLIER_COMPLETED_GOODS_FAIL ", e);
				Assert.fail(Constant.TEST_SUPPLIER_COMPLETED_GOODS_FAIL + " >> "
						+ e.getMessage());
			} catch (Exception e) {
				logger.error(Constant.TEST_SUPPLIER_COMPLETED_GOODS_FAIL, e);
				throw new TestNGException("Constant.TEST_SUPPLIER_COMPLETED_GOODS_FAIL >> "
						+ e.getMessage(), e);
			}
