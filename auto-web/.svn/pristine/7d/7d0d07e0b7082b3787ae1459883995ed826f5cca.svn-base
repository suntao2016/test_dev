...
package com.ccigmall.auto.test.reporter;
...
import com.ccigmall.auto.test.utils.ftp.FtpUtil;
import com.ccigmall.auto.test.utils.mail.HtmlMail;
...
public class HTMLReporter extends AbstractReporter {
	private static final String FRAMES_PROPERTY = "com.ccigmall.reportng.frames";
	private static final String ONLY_FAILURES_PROPERTY = "com.ccigmall.reportng.failures-only";

	private static final String TEMPLATES_PATH = "com/ccigmall/reportng/templates/html/";
	...
	private static void generateMailHtml(List<ISuite> suites, Date reportTime) {
		...
		String temp = ResourceBundle.getBundle("selenium")
				.getString("env");
		...
		sb.append("<h4>" + "详细结果：" + url + "</h4>");

		html.append(
				"<h4 style='color:red;'>" + env + "环境     测试  总计："
						+ (passed + fail + skip) + "\t通过：" + passed + "\t失败："
						+ fail + "\t跳过：" + skip + "\t通过率："
