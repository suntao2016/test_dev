 ...
 * 
 */
public class GoodsShelvesStep extends PlatformStep {
	/**
	 * slf4j
	 */
	private final static Logger logger = LoggerFactory
			.getLogger(GoodsShelvesStep.class);
	...
	public void goodsShelves() throws Exception {
		...
		try {
			...
			// 点击待出售的商品
			productListAct.clickPendingSale();
			// 输入上商品ID
			...
			// 点击上架按钮
			productListAct.clickGoodsShelves();
		} catch (AssertionError e) {
			logger.error("wofe商品上架 ", e);
			Assert.fail("wofe商品上架 >> " + e.getMessage());
		} catch (Exception e) {
			logger.error("wofe商品上架 ", e);
			throw new TestNGException("wofe商品上架 >> " + e.getMessage(), e);
		}
	}
}
