...
/**
 * 
* @ClassName: AddShoppingCartStep 
* @Description: 零售商--现货下单-加入购物车
* @author chenjingli 
* @date 2015-2-2 下午12:17:39
 */
public class AddSpotShoppingCartStep extends RetailerStep{
	/**
	 * slf4j
	 */
	private final static Logger logger = LoggerFactory
			.getLogger(AddSpotShoppingCartStep.class);
	
	protected static ProductDetailsAct detailsAct;
	/**
	 * 
	* @Title: testAddShoppingCart 
	* @Description: 
	* @param @throws Exception    
	* @return void 
	* @throws
	 */
	@Test(description = "pc 现货下单-加入购物车")
	public void testAddShoppingCart() {
		try {
			logger.info("零售商--现货下单-加入购物车测试用例执行开始，测试用例名称是：{}", getClass().getName());
			/**
			 * 初始化对象
			 */
			...
			logger.info("零售商--现货下单-加入购物车测试用例执行结束！");
		} catch (AssertionError e) {
			detailsAct.failScreenShot(Constant.TEST_RETAILER_ADD_SHOPPING_CART_FAIL);
			logger.error("Constant.TEST_RETAILER_ADD_SHOPPING_CART_FAIL ", e);
			Assert.fail(Constant.TEST_RETAILER_ADD_SHOPPING_CART_FAIL + " >> "
					+ e.getMessage());
		} catch (Exception e) {
			detailsAct.failScreenShot(Constant.TEST_RETAILER_ADD_SHOPPING_CART_FAIL);
			logger.error(Constant.TEST_RETAILER_ADD_SHOPPING_CART_FAIL, e);
			throw new TestNGException("Constant.TEST_RETAILER_ADD_SHOPPING_CART_FAIL >> "
					+ e.getMessage(), e);
		}
	}
