...
public class SpotBookingStep extends RetailerStep {
	 ...
	 * @throws Exception
	 */
	@Test(description = "pad retailer 零售商现货下单")
	@Parameters({ "moblie", "useID" ,"padUserName","padPassword"})
	public void testSpotBooking(String moblie, String useID,String padUserName,String padPassword) throws Exception {
		try {
			...
			String host = "http://192.168.0.122:8080/ccigmall-web-retailer";
			if (getMemVal(Constant.ENV).equals("sit")) {
				host = "http://192.168.0.122:8080";
			}
			driver.get(host + "/itemController/toItemDetail.do?PID="
					+ productId);
			logger.info("pad 零售商现货下单-测试用例执行开始，测试用例名称是：{}", getClass().getName());
			...
			setMemVal(Constant.PAYID, payId);
			 ...
			 //选择中国银行个人版
			 payAct.clickChinaBankPersonal();
			
			 //点击获取验证码
			 /* payAct.clickSendMobileCode();
			
			 //输入验证码
			 String code=getPadCode(moblie,useID);			
			 if(code==null){
			 payAct.pause(60);
			 //点击获取验证码
			 payAct.clickSendMobileCode();
			 code=getPadCode(moblie,useID);
			 }
			 //输入验证码
			 if(code==null){*/
				 UpdateOrderStatus.first(payId);
			/* }else{
				 payAct.enterMobileCode(code);
//				 //点击确认支付
			...
			logger.info("pad 零售商现货下单-测试用例执行结束！");
		} catch (AssertionError e) {
			logger.error("pad retailer spotbooking failed ", e);
			Assert.fail("pad retailer spotbooking failed >> " + e.getMessage());
		} catch (Exception e) {
			logger.error("pad retailer spotbooking failed ", e);
			throw new Exception("pad retailer spotbooking failed >> "
					+ e.getMessage(), e);
		}
